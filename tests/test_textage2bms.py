...
class TestTextage2Bms(unittest.TestCase):
    ...
    def test_main_runs_pipeline_and_quits_driver(self):
        ...
        with (
            patch.object(
                textage2bms, "argv", ["textage2bms.py", "https://example.com"]
            ),
            patch.object(textage2bms, "get_driver", return_value=driver),
            patch.object(textage2bms, "pq", return_value="DOC") as pq_mock,
            patch.object(
                textage2bms, "build_headers", return_value={"#TITLE": "T"}
            ) as build_headers_mock,
            patch.object(
                textage2bms, "get_sections", return_value=[[1, {}]]
            ) as get_sections_mock,
            patch.object(textage2bms, "print_header_field") as print_header_field_mock,
            patch.object(
                textage2bms, "print_main_data_field"
            ) as print_main_data_field_mock,
        ):
            textage2bms.main()
    ...
    def test_main_quits_driver_on_error(self):
        ...
        with (
            ...
            patch.object(
                textage2bms, "print_header_field", side_effect=RuntimeError("boom")
            ),
        ):
